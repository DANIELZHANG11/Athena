...
#### **2.7. 个人主页与阅读目标 (新增)**
-   **功能:** **对标 Apple Books**。展示阅读数据、目标追踪和个性化推荐。
-   **组件:**
    -   **ContinueReadingHero**: 顶部大卡片，展示最近阅读书籍及进度。
    -   **ReadingGoalCard**: 环形进度条展示今日阅读时长与目标，支持调整每日目标。
    -   **WeeklyActivity**: 7日阅读活动环形图。
    -   **YearlyGoalCard**: 年度书籍目标追踪，堆叠书籍封面展示已读数量。
-   **API:**
    -   `GET /api/v1/home/dashboard`: 获取聚合数据 (Streak, Goals, Recent Books)。
    -   `PATCH /api/v1/home/goals`: 更新每日/年度目标。
...
### **第五章：数据库 Schema 全量参考 (V9.1)**

> **注意**：本章汇总了从 0100 到 0113 所有 Migration 定义的最终 Schema 状态。

#### **5.1 用户与认证 (User & Auth)**
-   **`users`**: 核心用户表 (0100, 0105, 0110)
    -   `id` (UUID, PK)
    -   `email` (TEXT, Unique)
    -   `display_name` (TEXT)
    -   `is_active` (BOOLEAN)
    -   `membership_tier` (TEXT, Default: 'FREE')
    -   `language` (TEXT, Default: 'zh-CN') (0105)
    -   `timezone` (TEXT, Default: 'Asia/Shanghai') (0105)
    -   `membership_expire_at` (TIMESTAMPTZ) (0110)
    -   `monthly_gift_reset_at` (TIMESTAMPTZ) (0110)
    -   `free_ocr_usage` (INT) (0110)
    -   `version` (INT)
    -   `updated_at` (TIMESTAMPTZ)
-   **`user_sessions`**: 用户会话 (0100)
    -   `id` (UUID, PK)
    -   `user_id` (UUID, FK users)
    -   `created_at` (TIMESTAMPTZ)
-   **`user_stats`**: 用户资源统计 (0111, 0113)
    -   `user_id` (UUID, PK, FK users)
    -   `storage_used` (BIGINT, Default: 0) (0113)
    -   `book_count` (INT, Default: 0) (0113)
    -   `invite_count` (INT)
    -   `extra_storage_quota` (BIGINT)
    -   `extra_book_quota` (INT)
-   **`invites`**: 裂变邀请记录 (0111)
    -   `id` (UUID, PK)
...
-   **`reading_progress`**: 阅读进度 (0100)
    -   `id` (UUID, PK)
    -   `user_id` (UUID, FK users)
    -   `book_id` (UUID, FK books)
    -   `position` (TEXT): CFI or page number
    -   `progress` (FLOAT): 0.0 - 1.0
    -   `updated_at` (TIMESTAMPTZ)
-   **`user_reading_goals`**: 阅读目标 (0112)
    -   `user_id` (UUID, PK, FK users)
    -   `daily_minutes` (INT, Default: 30)
    -   `yearly_books` (INT, Default: 10)
    -   `updated_at` (TIMESTAMPTZ)
-   **`user_streaks`**: 连续阅读记录 (0112)
    -   `user_id` (UUID, PK, FK users)
...
#### **5.3 笔记与搜索 (Notes & Search)**
-   **`annotations`**: 高亮与笔记 (0100)
    -   `id` (UUID, PK)
    -   `user_id` (UUID, FK users)
    -   `book_id` (UUID, FK books)
    -   `cfi_range` (TEXT)
...
#### **5.4 AI 与 向量 (AI & Vectors)**
-   **`book_vectors`**: 书籍内容向量 (0100)
    -   `id` (UUID, PK)
    -   `book_id` (UUID, FK books)
    -   `chunk_index` (INT)
    -   `content` (TEXT)
    -   `embedding` (VECTOR[1536])
-   **`ocr_jobs`**: OCR 任务记录 (0100, 0111)
    -   `id` (UUID, PK)
    -   `user_id` (UUID, FK users)
    -   `book_id` (UUID, FK books)
    -   `status` (TEXT)
    -   `result_json` (JSONB)
    -   `page_count` (INT) (0111)
    -   `deduction_strategy` (TEXT) (0111)
    -   `deduction_amount` (INT) (0111)
    -   `created_at` (TIMESTAMPTZ)

#### **5.5 计费与会员 (Billing & Membership)**
-   **`pricing_rules`**: 价格策略 (0100, 0111)
    -   `id` (UUID, PK)
    -   `tier_name` (TEXT)
    -   `price_cents` (INT)
    -   `currency` (TEXT)
...
#### **5.6 系统与配置 (System & Config)**
-   **`system_config`**: 系统配置 (0100)
    -   `key` (TEXT, PK)
    -   `value` (JSONB)
-   **`runtime_stats`**: 运行时统计 (0103)
    -   `id` (UUID, PK)
    -   `node_id` (TEXT)
    -   `cpu_usage` (FLOAT)
    -   `memory_usage` (FLOAT)
