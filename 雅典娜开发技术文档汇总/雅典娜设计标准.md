...
- 系统色 Token 补齐与位置要求：
  - 在 `web/src/styles/figma.css` 的 `:root` 与 `.dark` 中定义真实值：
    - Light：`--color-system-blue: #007AFF`、`--color-label: rgba(0,0,0,0.85)`、`--color-secondary-label: rgba(60,60,67,0.6)`、`--color-system-fill: rgba(120,120,128,0.2)`
    - Dark：`--color-system-blue: #0A84FF`、`--color-label: rgba(255,255,255,0.85)`、`--color-secondary-label: rgba(235,235,245,0.6)`、`--color-system-fill: rgba(120,120,128,0.36)`
  - 禁止在文件末尾或非 `:root/.dark` 作用域定义上述变量，避免无效或覆盖
- 组件规范（Button）：
  - 页面禁止内联样式；统一复用组件
  - `primary` 变体：圆角 `999`、字重 `600`；颜色依赖系统 Token（`--color-system-blue`）
  - 可访问性：聚焦环、键盘可用（Tab/Enter/Space），对比度符合 WCAG AA
- 首页基线：CTA 使用统一 Button 组件；保留现有字号与动效参数，移除内联样式
- Tailwind v4 用法：重申 `@import "tailwindcss" + @theme inline + @layer base`，所有设计 Token 由 `figma.css` 下发
