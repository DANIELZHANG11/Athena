      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex justify-center items-end gap-2 md:gap-3 relative">
          {deviceImages.map((image, index) => {
            ...
            const initialY = 300
            return (
              <motion.div key={index} initial={{ x: initialX, y: initialY, opacity: 0 }} animate={{ x: 0, y: 0, opacity: 1 }} transition={{ duration: 1.4, delay: 2.0 + Math.abs(index - 2) * 0.12, ease: [0.22, 1, 0.36, 1] }} className="relative flex-shrink-0" style={{ width: '180px' }}>
                <motion.div className="relative rounded-[36px] overflow-hidden bg-black" style={{ y: index === 1 ? device2Y : index === 3 ? device4Y : 0, height: '360px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.4), 0 10px 20px -8px rgba(0, 0, 0, 0.3)' }}>
