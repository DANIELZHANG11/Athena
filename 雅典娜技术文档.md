...
| 认证 | JWT (RS256) + Refresh Tokens | OAuth + 邮箱验证码登录；Token管理：Redis；不提供手机号注册。 |
| 数据库 | PostgreSQL 15+ & pgvector | 统一存储关系数据和向量数据。 |
| 连接池 | PgBouncer | **[建议]** 高并发下减少连接建立开销；推荐会话池模式。
| 全文检索 | **Elasticsearch** | **[决策]** 提供强大的全文搜索、聚合、高亮和同义词能力。 |
| 对象存储 | MinIO (本地服务器部署) | S3兼容API，文件存储方案。 |
| 实时同步 | WebSocket + Yjs | 采用性能更优的Yjs实现CRDT无冲突同步。 |
| 后台任务 | **Celery + Redis Streams** | **[优化]** 使用Redis Streams作为消息代理，轻量且与现有Redis集成。 |
| 部署 | Docker Compose (本地服务器) | 简单、高效，满足MVP及早期生产阶段。 |
| AI - LLM | OpenRouter API（Anthropic/OpenAI/DeepSeek 等） | 统一路由聚合与计费，避免前期GPU硬件投入。 |
| AI - Embedding | Qwen-Embedding API | 采用API模式，成本极低。 |
| AI - OCR | 合合信息 (TextIn) API | 采用API模式，选择专业领域性价比最高的供应商。 |
| API网关 | **Traefik** | **[决策]** 轻量级，与Docker Compose原生集成，易于配置路由和限流。 |
| 密钥管理 | **Infisical** | **[决策]** 开源免费，替代Vault，与Docker Compose无缝集成。 |
| 可观测性 | Prometheus + Grafana + **Loki** + **Jaeger** | **[新增]** Loki用于日志聚合，与Prometheus/Grafana构成完整监控栈。 |
...
#### **4.1. 免费版**
-   书籍导入上限: 20本。
-   AI问答: 每月50次“与单本书对话”额度。
-   OCR: 0页。

#### **4.2. Pro专业版**
-   定价: 29元/月 或 298元/年。
-   权限: 书籍导入无限制，AI问答无限制，OCR每月赠送100页。
 -   赠送信用点：Pro专业版订阅用户，除了享有无限书籍导入等核心权益外，每月将自动获赠 2,000,000 雅典娜信用点，用于 AI 对话、OCR 等增值服务。当月未用完的赠送信用点将在下个月初清零。如果额度提前用完，Pro 用户可以随时额外购买信用点加油包。

#### **4.3. 增值服务**
-   OCR加油包、书籍导入加油包，通过 `credit_products` 和 `credit_transactions` 表进行管理。

#### **4.4. 会员等级与免费额度（动态配置）**
-   管理员通过 `system_settings.membership_tiers` 配置各等级的免费额度；服务端优先按用户 `membership_tier` 读取对应键，缺省时回退到全局 `free_*` 键。
-   示例：`{ "FREE": { "free_ocr_pages":0, "free_vector_chars":0 }, "PRO": { "free_ocr_pages":100, "free_vector_chars":300000 } }`。
-   计数与周期：`free_quota_usage` 按用户/服务/当日累计 `used_units`，每日重置。

#### **4.5. 计费与扣费策略（并行账本）**
-   钱包与信用点并行：`credit_accounts.balance`（Credits，整数）与 `wallet_amount/wallet_currency`（钱包金额，元）。
-   扣费顺序：免费额度 → 钱包（原币种） → Credits；所有消费记入 `credit_ledger`，钱包金额以“分”入账。
-   兑换：`POST /api/v1/billing/exchange`，汇率读取 `system_settings.wallet_exchange_rate`；支持多币种JSON映射，按 `wallet_currency` 选择。
-   定价读取：按 `pricing_rules(service_type/unit_type)` 最新有效规则计算，前端备注由模板占位符实时渲染。
...
### 2. 搜索索引与同步（Elasticsearch + Postgres）
- 索引：
  - `es.notes`：文档字段 `id,user_id,book_id,title,content,tag_ids,created_at`；启用高亮（`content`）。
  - `es.highlights`：字段 `id,user_id,book_id,text_content,color,chapter,tag_ids,created_at`；启用高亮（`text_content`）。
- 同步：
  - 写通道：`notes`/`highlights` 创建或更新后，异步写入ES（Celery任务）；删除标记 `deleted_at` 时从ES删除或打`is_deleted`过滤。
  - 失效：编辑笔记/高亮触发向量与缓存失效（与AI章节一致），同时更新ES文档。
- 备选：若ES不可用，回退到Postgres `tsvector` 检索（`notes.content_tsvector`、`highlights.text_content` 简单化分词）。
- 备选：若ES不可用，回退到Postgres `tsvector` 检索（`notes.content_tsvector`、`highlights.text_content_tsvector`）。
 - 混合检索（新增）：同时执行关键词检索（Elasticsearch）与向量ANN（pgvector），依据查询意图与置信度融合排序；对于包含特定术语的查询优先关键词结果。
...
- 全量重建/同步（新增）：
  ...
  def reindex_user_data(user_id: str):
      ...
      # 可选：清理索引中当前用户的陈旧文档
      es.purge_stale('notes', user_id, [n.id for n in notes])
      es.purge_stale('highlights', user_id, [h.id for h in highs])
  ```
...
paths:
  ...
  /api/v1/search:
    get:
      summary: 全局搜索（Notes/Highlights），支持ES或Postgres回退
      parameters:
        - in: query
          name: q
          schema: { type: string }
      ...
      responses:
        '200':
          description: OK
          headers:
            X-Search-Engine: { schema: { type: string, enum: [elasticsearch, postgres] } }
          content:
            application/json:
              schema:
                type: object
...
### 4. 后端逻辑伪代码（FastAPI/SQLAlchemy/ES）
```python
...
async def global_search(user_id, q, filters, limit, cursor):
    set_local_app_user(user_id)
    if es_available():
        res = await es_search(user_id, q, filters, limit, cursor)
...
async def es_search(user_id, q, filters, limit, cursor):
    should = []
    if 'note' in filters.types:
        should.append({ 'index': 'notes', 'query': build_es_query(q, user_id, filters) })
    if 'highlight' in filters.types:
        should.append({ 'index': 'highlights', 'query': build_es_query(q, user_id, filters) })
    hits = await es.multi_search(should, limit, cursor)
...
- E2E：
  ...
  - ES中断回退：关闭ES后搜索仍可返回基础结果，UI提示回退状态。
...
### 2. REST API 契约（OpenAPI 摘录，SSOT 见 contracts/api/v1/billing.yaml）
```yaml
openapi: 3.0.3
info: { title: Athena Billing, version: 1.0.0 }
servers: [ { url: / } ]
security: [ { bearerAuth: [] } ]
paths:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              type: object
              required: [product_id, gateway_provider_id]
              properties:
                product_id: { type: string, format: uuid }
                gateway_provider_id: { type: string, enum: [pingpong, stripe] }
      responses: { '200': { description: OK } }

  /api/v1/webhooks/pingpong:
    post:
      summary: PingPong 支付结果回调（无JWT）
      security: []
      responses: { '200': { description: OK }, '400': { description: INVALID_SIGNATURE } }
    ...
    get:
      ...
      parameters:
        ...
        - in: query
          ...
          schema: { type: string, enum: [PURCHASE, AI_CALL, REFUND, GRANT, CONSUME] }
        - in: query
          name: limit
          schema: { type: integer, default: 20, maximum: 100 }
        - in: query
...
### 6. 安全与合规
- 密钥管理：PingPong Webhook 签名密钥由 Infisical 安全管理；后端通过环境注入使用。
- 事务一致性：Webhook处理全链路使用数据库事务；失败自动回滚，避免账本与支付记录不一致。
- 审计与幂等：所有关键写操作携带 `Idempotency-Key`；Webhook事件ID防重；操作入 `audit_logs`；错误与重试采用指数退避。
...
### 二、词典与翻译（混合引擎模型）
#### 1. 核心架构
- 后端统一入口：新增 `/api/v1/dictionary` 模块；前端根据用户设置选择引擎。
...
- 在线翻译（Google/Microsoft）：前端调用后端代理接口（如 `POST /api/v1/dictionary/translate/microsoft`），后端再调用第三方 API；密钥仅在后端配置，严禁暴露前端。
- 语境感知 AI 翻译（付费）：
...
### 三、REST API 契约（OpenAPI 摘录，SSOT 见 contracts）
```yaml
openapi: 3.0.3
info: { title: Athena TTS & Dictionary, version: 1.0.0 }
servers: [ { url: / } ]
security: [ { bearerAuth: [] } ]
paths:
  /api/v1/dictionary/translate/microsoft:
    post:
      summary: 使用 Microsoft Translator 翻译
      requestBody:
        required: true
        content:
          application/json:
  ...
  /api/v1/dictionary/translate/google:
    post:
      summary: 使用 Google Translate 翻译
      requestBody:
        required: true
        content:
          application/json:
      ...
      requestBody:
        ...
        content:
          application/json:
            schema:
              type: object
              required: [text, source_language, target_language, model_id, context]
              properties:
                text: { type: string }
                source_language: { type: string }
                target_language: { type: string }
          ...
          application/json:
            schema:
              ...
              properties:
                word: { type: string }
                book_id: { type: string, format: uuid }
                engine: { type: string, enum: [offline, microsoft, google, ai] }
                source_language: { type: string }
                target_language: { type: string }
                result: { type: object }
      responses: { '201': { description: Created } }
