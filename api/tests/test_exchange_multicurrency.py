...
# NOTE: Admin/Billing API 测试保留 REST 调用是合理的，因为管理后台本身就是 Web-First。
...
async def test_exchange_wallet_multicurrency(monkeypatch):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        r = await client.post(
            ...
            json={"email": "test@athena.local", "code": dev_code},
        )
        assert r.status_code == 200
        ...
        r = await client.post(
            ...
            headers=auth,
            json={"gateway": "fake", "amount": 1000, "currency": "CNY"},
        )
        assert r.status_code == 200
        sid = r.json()["data"]["id"]
        ...
        r = await client.post(
            ...
            headers=auth,
            json={"direction": "credits_to_wallet", "amount": 1000},
        )
        assert r.status_code == 200
        r = await client.get("/api/v1/billing/balance", headers=auth)
        assert r.status_code == 200
