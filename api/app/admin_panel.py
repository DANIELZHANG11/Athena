...
async def get_settings(auth=Depends(require_user)):
    ...
    try:
        ...
        return {"status": "success", "data": [{"key": k, "value": v, "updated_at": ""} for k, v in _MEM_SETTINGS.items()]}
...
async def list_providers(service_type: str | None = None, auth=Depends(require_user)):
    ...
    try:
        ...
        rows = []
        for (st, nm), v in _MEM_PROVIDERS.items():
            if service_type and st != service_type:
                continue
            rows.append({"id": v.get("id") or f"mem-{st}-{nm}", "service_type": st, "name": nm, "endpoint": v.get("endpoint"), "config": v.get("config") or {}, "is_active": bool(v.get("is_active", True)), "priority": int(v.get("priority", 0)), "etag": "W/\"1\"", "updated_at": ""})
...
async def upsert_provider(body: dict = Body(...), auth=Depends(require_user)):
    ...
    try:
        ...
        nm = body.get("name")
        _MEM_PROVIDERS[(st, nm)] = {"id": f"mem-{st}-{nm}", "endpoint": body.get("endpoint"), "config": body.get("config") or {}, "is_active": body.get("is_active", True), "priority": body.get("priority", 0)}
        return {"status": "success"}
...
async def list_models(auth=Depends(require_user)):
    ...
    try:
        ...
        return {"status": "success", "data": [{"id": k, "provider": v.get("provider"), "model_id": v.get("model_id"), "display_name": v.get("display_name"), "active": bool(v.get("active", True)), "updated_at": ""} for k, v in _MEM_MODELS.items()]}
...
async def upsert_model(body: dict = Body(...), auth=Depends(require_user)):
    ...
    try:
        ...
        _MEM_MODELS[body.get("model_id") or str(uuid.uuid4())] = {"provider": body.get("provider"), "model_id": body.get("model_id"), "display_name": body.get("display_name"), "active": body.get("active", True)}
        return {"status": "success"}
